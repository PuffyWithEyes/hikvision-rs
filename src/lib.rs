//! # hikvision-rs
//!
//! High-level asynchronous library for controlling cameras from Hikvision using
//! the PTZ API ```rust
//! #[tokio::main]
...
/// The structure of the camera allows you to communicate with it at a high
/// level
#[derive(Clone, Debug)]
pub struct Cam {
    address: String,
...
impl Cam {
    /// Creating an object to connect to the camera. If there is no login and
    /// password, then the `user_passwd` field should have the value `None`
    pub async fn new<S>(
        addr: S,
        port: usize,
        ...
        S: Into<String>,
    {
        let (addr, test_addr) = match user_passwd {
            Some((user, passwd)) => {
                ...
                (
                    format!(
                        "http://{}:{}@{}:{}/ISAPI/PTZCtrl/channels/1/Momentary",
                        user, passwd, addr, port
                    ),
                    format!(
                        "http://{}:{}@{}:{}/ISAPI/PTZCtrl/channels/1/capabilities",
                        user, passwd, addr, port
                    ),
                )
            },
        ...
        if test_conn.contains("Document Error: Unauthorized") {
    ...
    async fn send_data(&mut self) -> Result<Response, Error> {
        self.client
            ...
            .body(format!(
                ...
                self.pan.data, self.tilt.data, self.zoom.data, self.movement_speed
            ))
            .send()
            .await
    }
    ...
    async fn cam_event(
        ...
        if time.duration_since(event.last_trigger).as_millis() + 50
            < time::Duration::from_millis(self.movement_speed as u64).as_millis()
            && !event.is_init
        {
            return Err(Box::new(error::QuickRequsetError::new(
                self.movement_speed,
                type_event,
            )));
        } else {

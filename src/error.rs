use crate::TypeEvent;
/// `ErrorAuthorize`, usually occurs when the login or password is incorrect or
/// due to the lack of certain access rights to the camera
use std::fmt;
...
impl fmt::Display for ErrorAuthorize {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "Failed to log in and access the camera. Check that your login and password are \
             correct, and also check on the website in the Configuration -> Video/Audio -> \
             \"Stream Type\" must be set to \"Sub-stream\". Also on this page set the value of \
             the \"Video Encoding\" item to MPJPEG"
        )
    }
}
...
/// `QuickRequestError` usually occurs because you try to send the same action
/// to the camera very quick
...
impl fmt::Display for QuickRequsetError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "You are making request for the <{}> action too quickly. {}ms must have passed since \
             the last request",
            self.event.get_str(),
            self.timeout
        )
...
impl fmt::Display for OutOfRangeUnitError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "The unit of measurment for the <{}> event does ot lie in the range -100..=100, its \
             value {}",
            self.event.get_str(),
            self.data
        )
